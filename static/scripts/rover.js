...
const focusNextLine = (rover, direction) => {
  const index = [...state.entries()]
    .filter(([key]) => key instanceof Element)
    .findIndex(([key, value]) => rover.isEqualNode(key))
  
  // Up
  if (direction === KEYCODE.DOWN && index < state.size) {
    const el = Array.from(state)[index + 1]
    const e = {
      currentTarget: el.at(0)
    }
    onFocusin(e)
  }
  // Down
  if (direction === KEYCODE.UP && index > 0) {
    const el = Array.from(state)[index - 1]
    const e = {
      currentTarget: el.at(0)
    }
...
const onKeydown = e => {
  ...
  switch (e.keyCode) {
    case KEYCODE.UP:
      e.preventDefault()
      focusNextLine(rover, KEYCODE.UP)
      break
    case KEYCODE.DOWN:
      e.preventDefault()
      focusNextLine(rover, KEYCODE.DOWN)
      break
    case KEYCODE.RIGHT:
      e.preventDefault()
      focusNextItem(rover)
